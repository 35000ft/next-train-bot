...
async def handle_query_emu_no(message: GroupMessage | C2CMessage, train_no: str, **kwargs):
    if train_no[0:1] not in ['D', 'C', 'G', 'S']:
        return await message.reply(content=f'不支持的车次:{train_no}')
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            content = resp.json()
            if not content:
                return await message.reply(content=f'没有查到车次{train_no}的动车组担当信息')
    except Exception as e:
        logger.exception(e)
        return await message.reply(content=f'查询车次动车组担当失败:{train_no}')
    ...
    if target_date_record:
        ...
        return await message.reply(content=r_content)
