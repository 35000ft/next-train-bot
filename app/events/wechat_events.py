...
async def handle_query_wechat_article(message: GroupMessage, keyword: str, **kwargs):
    async with get_db_session() as session:
        articles: List[Article] = await query_articles(session, keyword, partition=message.group_openid)
    if articles:
...
@authorize(permission_key='ADD-WECHAT_REPLY-ARTICLE', param_getter=lambda message: message.group_openid)
# TODO 自定义关键词
...
async def handle_add_auto_reply_wechat_article(message: GroupMessage, url: str, keyword: str = None, **kwargs):
    ...
    article_model = WechatArticle(title=article.Title, description=article.Description, partition=message.group_openid,
